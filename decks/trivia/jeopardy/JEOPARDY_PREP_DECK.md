...
python -m decks.trivia.jeopardy.smart_prep jeopardy_smart_prep.colpkg jeopardy_smart_prep.apkg
...
python -m decks.trivia.jeopardy.study_optimizer

# 4. One-time: make new cards show before reviews (see Study Queue Ordering)
python -m decks.trivia.jeopardy.configure_deck_options --preset Jeopardy
```
...
python -m decks.trivia.jeopardy.smart_prep --live-db "~/.local/share/Anki2/User 1/collection.anki2"
python -m decks.trivia.jeopardy.archive_dead_cards     # park dead cards in the Archive subdeck
python -m decks.trivia.jeopardy.study_optimizer        # ease tuning + weakness-weighted new-card order
```
...
python -m decks.trivia.jeopardy.smart_prep --live-db "$DB" --analysis-only --evidence-report /tmp/jeopardy-moves.tsv
...
python -m decks.trivia.jeopardy.smart_prep --live-db "$DB"

# 4. Prove nothing but the badge, details and tags changed (exits 1 otherwise).
python -m decks.trivia.jeopardy.verify_refresh compare "$DB" /tmp/jeopardy-before.json
```
...
```bash
python -c "from pathlib import Path; from decks.trivia.jeopardy.jeopardy_db_helpers import connect_anki; \
...
`python -m decks.trivia.jeopardy.archive_dead_cards --restore` moves every archived card back.

---

## Scripts & Files

Everything below lives in this folder (`decks/trivia/jeopardy/`) and runs as a module from the repository root: `python -m decks.trivia.jeopardy.<name>` (no `.py`). Data files are in `data/`; scripts locate them relative to their own location, so the current directory no longer matters.
...
| `tests/`                       | `python -m pytest decks/trivia/jeopardy/tests -q` — reclassification rule, per-card subjects, end-to-end refresh, the verify guard |
...
| `data/category_taxonomy.json`  | LLM classification cache: `{CATEGORY: {subject, sub_category, secondary_subject}}`                           |
| `data/combined_season1-41.tsv` | jwolle1 clue dataset, Seasons 1-41 (input to `update_collection.py`)                                         |
...
> **This setting lives on the preset, not the deck — creating a new preset silently reverts it.** Assigning `Jeopardy Smart Prep` to a freshly-created options preset in the Anki GUI starts it from Anki's stock defaults ("mix with reviews", learn 1m/10m), regardless of what the old preset had. This is not theoretical: it happened between 2026-07-29 and 2026-08-01 and made a study session look like the grouping fix had failed, when the `due` order was in fact correct the whole time. If groups start getting interrupted again, check the preset first with `python -m decks.trivia.jeopardy.configure_deck_options --dry-run` — it reports the current values for every preset and warns if the gather/sort order would break `due` ordering.
