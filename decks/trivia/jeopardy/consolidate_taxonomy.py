...
Run AFTER classify_categories.py finishes:
    python -m decks.trivia.jeopardy.consolidate_taxonomy [--dry-run] [--input category_taxonomy.json]
...
from decks.trivia.jeopardy.jeopardy_consts import SUBJECT_OTHER, TAXONOMY_PATH_DEFAULT
...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=Path(TAXONOMY_PATH_DEFAULT),
        help="Input taxonomy JSON (default: data/category_taxonomy.json beside this script)",
    )
    parser.add_argument(
        "--output",
        type=Path,
